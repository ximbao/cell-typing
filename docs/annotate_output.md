...
| `min_counts` | `20` | QC threshold: cells with `total_counts < min_counts` are flagged `low_quality` and excluded from smoothing, scoring and the benchmark. Counts come from `layers["counts"]` if `X` is already normalised. |
